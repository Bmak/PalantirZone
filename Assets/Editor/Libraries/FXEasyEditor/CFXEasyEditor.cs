...
public class FXEasyEditor : EditorWindow
{
	...
	{
		//Load Settings
		pref_ShowAsToolbox = EditorPrefs.GetBool("FX_ShowAsToolbox", true);
		pref_IncludeChildren = EditorPrefs.GetBool("FX_IncludeChildren", true);
		basicFoldout = EditorPrefs.GetBool("FX_BasicFoldout", false);
		colorFoldout = EditorPrefs.GetBool("FX_ColorFoldout", false);
		copyFoldout = EditorPrefs.GetBool("FX_CopyFoldout", false);
	}
	
	void OnDisable()
	{
		//Save Settings
		EditorPrefs.SetBool("FX_BasicFoldout", basicFoldout);
		EditorPrefs.SetBool("FX_ColorFoldout", colorFoldout);
		EditorPrefs.SetBool("FX_CopyFoldout", copyFoldout);
	}
	...
	void OnGUI()
	{
		...
		GUILayout.Label("FX Easy Editor", EditorStyles.boldLabel);
		...
		if(GUI.changed)
		{
			EditorPrefs.SetBool("FX_ShowAsToolbox", pref_ShowAsToolbox);
			this.Close();
			FXEasyEditor.ShowWindow();
		}
	...
	//----------------------------------------------------------------
		...
		if(GUI.changed)
		{
			EditorPrefs.SetBool("FX_IncludeChildren", pref_IncludeChildren);
		}
...
//		GUILayout.Space(6);
	...
	//----------------------------------------------------------------
		...
		if(copyFoldout)
		{
			...
			if(GUILayout.Button("Copy properties to selected Object(s)"))
			{
				...
				if(!foundPs)
				{
					Debug.LogWarning("FX Easy Editor: No Particle System found in the selected GameObject(s)!");
				}
			}
		}
	...
	private void tintColor()
	{
		...
		{
			Debug.LogWarning("FX Easy Editor: You must toggle at least one of the three Color Modules to be able to tint anything!");
			return;
		}
	...
	private void addLightness(bool substract)
	{
		if(!TintStartColor && !TintColorModule && !TintColorSpeedModule)
		{
			Debug.LogWarning("FX Easy Editor: You must toggle at least one of the three Color Modules to be able to change lightness!");
			return;
		}
	...
	private void applyDelay()
	{
		foreach(GameObject go in Selection.gameObjects)
		{
			...
			{
				ParticleSystem.MainModule main = ps.main;
				main.startDelayMultiplier = DelayValue;
			}
		}
	}
	...
	private void CopyModules(ParticleSystem source, ParticleSystem dest)
	{
		if(source == null)
		{
			Debug.LogWarning("FX Easy Editor: Select a source Particle System to copy properties from first!");
			return;
		}
		...
		ParticleSystem.MainModule destMain = dest.main;
		...
		if(b_modules[0])
		{
			...
			destMain.startDelay = sourceMain.startDelay;
			destMain.loop = sourceMain.loop;
			destMain.playOnAwake = sourceMain.playOnAwake;
			destMain.simulationSpeed = sourceMain.simulationSpeed;
			destMain.startSpeed = sourceMain.startSpeed;
			destMain.startSize = sourceMain.startSize;
			destMain.startColor = sourceMain.startColor;
			destMain.startRotation = sourceMain.startRotation;
			destMain.startLifetime = sourceMain.startLifetime;
			destMain.gravityModifier = sourceMain.gravityModifier;
			ParticleSystem.EmissionModule em = dest.emission;
			em.rateOverTime = source.emission.rateOverTime;
		}
...
//		ps.main.gravityModifier = ScalingValue;
