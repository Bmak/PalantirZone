...
public class UISystem : MonoBehaviour, IInitializable, ILoggable
{
	...
	private const string UI_LAYER_MAP_OBJECTS = "MapObjects";
...
*/
	...
	private void _createCameraComponents()
	{
		...
		//_uiCamera.eventReceiverMask = ~(1 << LayerMask.NameToLayer(UI_LAYER_FRONT_NAME)); // VPaint fix, so input doesn't get blocked by VPaint colliders
        string[] layers = new string[] { UI_LAYER_FRONT_NAME, UI_LAYER_NAME };
        _uiCamera.eventReceiverMask = LayerMask.GetMask(layers);
		_uiCamera.allowMultiTouch = true;
		_uiCamera.useController = false;
	    _uiCamera.useKeyboard = false;
	    _uiCamera.eventsGoToColliders = true;
	...
	public void _setInputEnabled(bool enabled)
	{
		...
		_uiCamera.allowMultiTouch = enabled;
	...
	public void UpdateProgressLoading(float value, string loadName)
	{
		_backdrop.UpdateProgressValue(value, loadName);
	}
